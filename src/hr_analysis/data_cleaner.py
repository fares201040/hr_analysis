...
def clean_all_csvs() -> pd.DataFrame:
    """
    Cleans all CSV files in unclean_data (only once per process):
    ...
    for f in csv_files:
        ...
        if "employee_id" in df.columns and "date" in df.columns:
            df["employee_date_id"] = (
                df["employee_id"].astype(str) + "_" + df["date"].astype(str)
            )
...
#                             "ignore", category=UserWarning, module="pandas"
#                         )
#                         return pd.to_datetime(val)
#                 except Exception:
#                     return val
...
#             )
#         else:
#             df["employee_date_id"] = df.index.map(
#                 lambda x: f"unidentified_{i}_{str(x)}"
#             )
#     # Concatenate all cleaned DataFrames
...
#         merged_df["employee_date_id"] = (
#             merged_df["employee_id"].astype(str) + "_" + merged_df["date"].astype(str)
#         )
#         merged_df.set_index("employee_date_id", inplace=True)
#     clean_dir = base_dir / "clean_data"
#     clean_dir.mkdir(exist_ok=True)
