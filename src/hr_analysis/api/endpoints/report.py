...
def overtime_weekly_summary(
    ...
    if "total_ot_rounded" in df.columns:
        df = df[df["total_ot_rounded"].fillna(0) > 0].copy()
    else:
        # If the column is missing, return an empty result as we can't calculate OT.
        return {"overtime_weekly_summary": [], "columns": ["employee_id"]}
    # Week calculation
    if week_start.lower() == "monday":
        ...
        # We want to find the date of the Sunday for the week containing the date.
        # (date.weekday + 1) % 7 maps Sunday to 0, Monday to 1, ..., Saturday to 6.
        # Subtracting this many days from the current date gives the Sunday of that week.
        df["week_start_date"] = df["date"] - pd.to_timedelta(
            (df["date"].dt.weekday + 1) % 7, unit="d"
        )
        # Format the week label using the start date. Using ISO week format for consistency in labeling.
        df["week_label"] = df["week_start_date"].dt.strftime("%Y-W%V")
